...
struct bq27541_device_info {
	...
	bool set_screen_off;
	struct work_struct screen_state_worker;
...
static void bq27541_set_lcd_off_status(struct work_struct *work)
{
	int soc,ret;

	if (bq27541_di) {
		bool off = bq27541_di->set_screen_off;
		...
		unsigned long action, void *data)
{
	...
	di->set_screen_off = *blank != FB_BLANK_UNBLANK;
	schedule_work(&di->screen_state_worker);
	return NOTIFY_OK;
}
		...
		const struct i2c_device_id *id)
{
	...
	bq27541_parse_dt(di);
	di->lcd_is_off = false;
	INIT_WORK(&di->screen_state_worker, bq27541_set_lcd_off_status);
...
static void bq27541_shutdown(struct i2c_client *client)
{
	...
	if (di->already_modify_smooth)
		bq27411_modify_soc_smooth_parameter(di, false);

	smblib_backup_soc(di->soc_pre);
}
